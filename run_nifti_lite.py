...
def pred_lite(path, test):
    # Load the TFLite model and allocate tensors.
    interpreter = tf.lite.Interpreter(model_path=path, num_threads=8)
    # Get input and output tensors.
    input_details = interpreter.get_input_details()
    output_details = interpreter.get_output_details()
    # Test the model on input data.
    interpreter.resize_tensor_input(input_details[0]['index'],test.shape )
...
def slicer(volume, spacing, folder, DATA_DIR, MODEL_FILE):
    ...
    print('Predicting...')
    results = pred_lite(MODEL_FILE, TEST_SET)
    print('Done!')
    # Select slices
...
def main(args):
	...
	MODEL_SLICE    = './slicer/weights/VGG_19_quant.tflite' #'/var/www/html/compositia/proc/VGG_19.hdf5'
	MODEL_L1       = './L1scripts/weights/unet_DB1_mcL1_quant.tflite' #'/var/www/html/compositia/proc/unet_DB1_mcL1.hdf5'
	MODEL_L3       = './L3scripts/weights/unet_DB1_multi_quant.tflite' #'/var/www/html/compositia/proc/unet_DB1_multi_256.hdf5'
	...
	## L1 segmentation
	cort, L1_mask = L1segmentation(L1_slice, DATA_DIR, MODEL_L1)
	## L3 segmentation
