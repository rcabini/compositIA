...
def main():
    ...
    path = '../../DATA_input/L1/test/image'
    seg_path = '../../DATA_input/L1/test/label'
    res_path = './res'
    cort_path = './cort'
    spung_path = './spung'

    print('Loading model ... ')
    model = keras.models.load_model('./weights/unet_DB1_mcL1.hdf5', custom_objects={"dice_coef": dice_coef })
    ...
    for file in glob.glob(path+"/*.png"):
        ...
        #X = trans.resize(X, (1, input_size[0], input_size[1], 1), mode='constant',cval=0,anti_aliasing=True,preserve_range=True,order=3)
        
        mask = load_img(seg_path + '/' + fn, grayscale=False)
        x_mask = img_to_array(mask)
        x_mask = x_mask.squeeze() / 255
        ...
        res_seg = res_seg*255
        matplotlib.image.imsave(cort_path+ '/' + fn, res_seg[:,:,1].astype(np.uint8),  cmap="gray")
        matplotlib.image.imsave(spung_path+ '/' + fn, res_seg[:,:,2].astype(np.uint8),  cmap="gray")
